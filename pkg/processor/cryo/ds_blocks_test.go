...
func TestDecodeBlocks(t *testing.T) {
	...
	require.Equal(t, "0x396343362be2a4da1ce0c1c210945346fb82aa49", *r.Author)
	require.Equal(t, uint64(11210841), *r.GasUsed)
	require.Equal(t, uint64(45000000), r.GasLimit)
	require.Equal(t, uint64(239712557), *r.BaseFeePerGas)
	require.Equal(t, "mainnet", r.MetaNetworkName)

	// extra_data is stored twice: verbatim hex, and the raw bytes it encodes.
	require.Equal(t, "0xe29ca82051756173617220287175617361722e77696e2920e29ca8", *r.ExtraData)
	require.Equal(t, "✨ Quasar (quasar.win) ✨", *r.ExtraDataString)
