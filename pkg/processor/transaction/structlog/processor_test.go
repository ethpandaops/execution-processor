...
func TestHandleBlockNotFound_DiffCalculation(t *testing.T) {
	// This test verifies the correct behavior of handleBlockNotFound:
	// - Blocks at or behind chain tip (diff <= 0) should NOT trigger "not yet available"
	// - Blocks 1-5 ahead of chain tip should trigger "not yet available" (might appear soon)
	// - Blocks >5 ahead of chain tip should return "block not found" (something is wrong)
	//
	// The fixed logic:
	//   diff := nextBlock - chainTip
	//   if diff > 0 && diff <= 5 { return "block not yet available" }
	testCases := []struct {
		name                 string
		nextBlock            int64
		chainTip             int64
		expectedDiff         int64
		shouldBeAvailable    bool // true = block should exist (at or behind tip)
		shouldWaitForBlock   bool // true = "not yet available" message (1-5 ahead)
		shouldReturnNotFound bool // true = "block not found" message (>5 ahead)
	}{
		{
			name:                 "block far behind chain tip - should exist",
			nextBlock:            24365273,
			chainTip:             24365743,
			expectedDiff:         -470,
		...
		{
			name:                 "block 100 blocks behind chain tip - should exist",
			nextBlock:            100,
			chainTip:             200,
			expectedDiff:         -100,
			shouldBeAvailable:    true,
		...
		{
			name:                 "block at chain tip - should exist",
			nextBlock:            200,
			chainTip:             200,
			expectedDiff:         0,
			shouldBeAvailable:    true,
		...
		{
			name:                 "block 1 ahead of chain tip - wait for it",
			nextBlock:            201,
			chainTip:             200,
			expectedDiff:         1,
			shouldBeAvailable:    false,
			shouldWaitForBlock:   true,
			shouldReturnNotFound: false,
		},
		{
			name:                 "block 5 ahead of chain tip - wait for it",
			nextBlock:            205,
			chainTip:             200,
			expectedDiff:         5,
			shouldBeAvailable:    false,
		...
		{
			name:                 "block 6 ahead of chain tip - not found",
			nextBlock:            206,
			chainTip:             200,
			expectedDiff:         6,
			shouldBeAvailable:    false,
		...
		{
			name:                 "block 10 ahead of chain tip - not found",
			nextBlock:            210,
			chainTip:             200,
			expectedDiff:         10,
			shouldBeAvailable:    false,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			nextBlock := big.NewInt(tc.nextBlock)
			chainTip := big.NewInt(tc.chainTip)

			// Calculate diff as the code does
			diff := new(big.Int).Sub(nextBlock, chainTip).Int64()

			assert.Equal(t, tc.expectedDiff, diff, "Diff calculation should match")

			// Fixed behavior: diff > 0 && diff <= 5 means "not yet available"
			fixedCodeWaitsForBlock := diff > 0 && diff <= 5
			fixedCodeReturnsNotFound := diff > 5
			fixedCodeSaysAvailable := diff <= 0

			assert.Equal(t, tc.shouldWaitForBlock, fixedCodeWaitsForBlock,
				"Fixed code should wait for block when 1-5 blocks ahead")
			assert.Equal(t, tc.shouldReturnNotFound, fixedCodeReturnsNotFound,
				"Fixed code should return 'not found' when >5 blocks ahead")
			assert.Equal(t, tc.shouldBeAvailable, fixedCodeSaysAvailable,
				"Fixed code should say available when at or behind chain tip")

			// Verify mutual exclusivity
			outcomes := 0

			if fixedCodeWaitsForBlock {
				outcomes++
			}

			if fixedCodeReturnsNotFound {
				outcomes++
			}

			if fixedCodeSaysAvailable {
				outcomes++
			}
