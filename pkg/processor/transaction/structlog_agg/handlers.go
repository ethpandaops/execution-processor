...
func (p *Processor) handleProcessForwardsTask(ctx context.Context, task *asynq.Task) error {
	...
	// Process transaction - aggregate call frames
	frameCount, err := p.ProcessTransaction(ctx, block, int(payload.TransactionIndex), tx)
	if err != nil {
	...
	// Track block completion using embedded Limiter
