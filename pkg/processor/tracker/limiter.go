...
func (l *Limiter) IsBlockedByIncompleteBlocks(
	...
	maxPendingBlockRange := uint64(l.config.MaxPendingBlockRange) //nolint:gosec // G115: validated positive above
...
func (l *Limiter) GetGaps(ctx context.Context, currentBlock uint64, lookbackRange uint64, limit int) (*GapResult, error) {
	...
	if l.config.MaxPendingBlockRange > 0 {
		exclusionWindow := uint64(l.config.MaxPendingBlockRange) //nolint:gosec // G115: validated positive above
